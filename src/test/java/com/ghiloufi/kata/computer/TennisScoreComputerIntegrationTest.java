...
class TennisScoreComputerIntegrationTest {
  ...
  void testCompleteDeuce() {
    ...
    assertTrue(history.get(history.size() - 1).equals("Player A : 40 / Player B : 40 (Deuce)"));
  }
  ...
  void testLongDeuceGame() {
    ...
    long deuceCount = history.stream().filter(line -> line.contains("(Deuce)")).count();

    assertTrue(deuceCount >= 3, "Should have at least 3 deuce situations");
    assertTrue(
        history
            .get(history.size() - 1)
            .equals("Player A : 40 / Player B : 40 (Advantage Player A)"));
  }
  ...
  void testScoreFormatConsistency() {
    ...
    for (String score : history) {
      if (!score.contains("wins")) {
        assertTrue(
            score.matches("Player A : (\\d+|40) / Player B : (\\d+|40).*"),
