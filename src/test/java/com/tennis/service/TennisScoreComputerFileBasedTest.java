...
/**
 * File-based tests reading test cases from external resource file
 */
    ...
    void testCasesFromFile() {
        ...
        for (TestCase testCase : testCases) {
            ...
            if ("DEUCE".equals(testCase.expectedOutcome)) {
                assertEquals("DEUCE", computer.getGameState().name(), 
                    "Test case failed: " + testCase.description);
            } else if ("A".equals(testCase.expectedOutcome)) {
                assertEquals("GAME_WON_A", computer.getGameState().name(), 
                    "Test case failed: " + testCase.description);
            } else if ("B".equals(testCase.expectedOutcome)) {
                assertEquals("GAME_WON_B", computer.getGameState().name(), 
                    "Test case failed: " + testCase.description);
            }
        }
    ...
    private List<TestCase> loadTestCasesFromFile() {
        ...
        try {
            InputStream inputStream = getClass().getClassLoader()
                .getResourceAsStream("test-cases.txt");
